...
export function HiddenJobsLanding() {
  ...
  const heroPreview = previewJobs.slice(0, 4);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return previewJobs.filter((j) => {
      if (role !== "All roles" && j.roleCategory !== role) return false;
      if (!q) return true;
      const blob = `${j.company} ${j.title} ${j.location} ${j.roleCategory}`.toLowerCase();
      return blob.includes(q);
    });
  }, [query, role, previewJobs]);

  const totalLabel = stats?.totalActive ?? previewJobs.length;
  const companiesLabel = stats?.companies ?? 0;
          ...
          <div className="landing-hero-stagger text-center lg:text-left">
            ...
            <div className="mx-auto mb-8 grid max-w-md gap-3 sm:grid-cols-2 lg:mx-0 lg:max-w-none">
              <div className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 text-left shadow-sm backdrop-blur-sm">
                ...
                <p className="mt-1 text-xs leading-relaxed text-slate-500">Apply on the company site, not a third-party board.</p>
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 text-left shadow-sm backdrop-blur-sm">
            ...
            <p className="mt-5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-slate-500 lg:justify-start">
              <span>
                <span className="font-semibold text-slate-700">{totalLabel}</span> US internships
              </span>
              {companiesLabel > 0 && (
                <>
                  ...
                  <span>
                    <span className="font-semibold text-slate-700">{companiesLabel}</span> companies
                  </span>
                </>
              )}
            </p>
              ...
              <div className="rounded-[1.35rem] bg-gradient-to-br from-scale-lavender/90 via-white to-slate-50 p-6 sm:p-8">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-slate-900">USA internships</p>
                  </div>
                ...
                <div className="space-y-2.5">
                  ...
                  {!loading && heroPreview.length === 0 && (
                    <p className="text-xs text-slate-500 py-4 text-center">
                      Internships sync daily. Check back soon.
                    </p>
                  )}
                </div>
              </div>
